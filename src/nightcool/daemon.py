...
def run_once(
    ...
    if provider is None:
        ...
        # Persist any newly-cached coordinates onto a fresh read so we don't
        # clobber keys the web process wrote after our snapshot.
    ...
    if should_notify(rec, last, now, schedule=cfg.schedule, prefs=cfg.prefs):
        ...
        # Re-read before writing: the top-of-cycle snapshot is stale after the
        # forecast fetch and push sends, and writing it back would drop any
        # subscriptions or indoor temps the web process saved meanwhile.
        state = read_state(state_path)
