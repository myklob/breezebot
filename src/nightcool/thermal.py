...
def _solve_least_squares(rows: list[tuple[float, float, float, float]]) -> tuple[tuple[float, float, float], float] | None:
    ...
    Returns ((alpha, beta, gamma), r_squared), or None when the design
    matrix is singular (e.g. a feature column is identically zero) — the
    data can't identify the coefficients, so there is no fit to report.
    """
    n = len(rows)
