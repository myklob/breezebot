...
def test_toggling_home_all_day_off_preserves_leave_at(tmp_path):
    # A day that has both a leave_at and home_all_day: true. Un-ticking
    # "home all day" (payload sends only home_all_day: false) must NOT erase
    # the stored departure time.
    state = tmp_path / "state.json"
    state.write_text("{}")
    config_path = tmp_path / "config.yaml"
    config_path.write_text(
...
location: {latitude: 39, longitude: -104, timezone: UTC}
windows: [{id: w, name: W, exposure: exposed, security: secure}]
schedule: {mon: {target_f: 68, leave_at: "07:30", home_all_day: true}}
""".strip()
    )
    from nightcool.config import load_config
    cfg = load_config(config_path)
